...
HOSTNAME=$(sshpass ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=accept-new Administrator@${WINDOWS_HOST} hostname)
...
sshpass ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=accept-new Administrator@${WINDOWS_HOST} 'powershell.exe -Command "secedit /export /cfg setup.inf"'

# Transfer the file directly to the web server using sshpass
echo "Transferring file to the web server..."
sshpass scp -i ~/.ssh/id_rsa -o StrictHostKeyChecking=accept-new Administrator@${WINDOWS_HOST}:setup.inf ${SCAN_DIR}/${FILENAME}
