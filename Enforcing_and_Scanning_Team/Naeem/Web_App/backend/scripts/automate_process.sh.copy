...
HOSTNAME=$(sshpass -p '1qwer$#@!' ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=accept-new ${WINDOWS_USER}@${WINDOWS_HOST} hostname)
...
sshpass -p '1qwer$#@!' ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=accept-new ${WINDOWS_USER}@${WINDOWS_HOST} 'powershell.exe -Command "secedit /export /cfg setup.inf"'

# Transfer the file directly to the web server using sshpass
echo "Transferring file to the web server..."
sshpass -p '1qwer$#@!' scp -i ~/.ssh/id_rsa -o StrictHostKeyChecking=accept-new ${WINDOWS_USER}@${WINDOWS_HOST}:setup.inf ${SCAN_DIR}/${FILENAME}
